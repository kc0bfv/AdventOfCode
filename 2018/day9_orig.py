...
if __name__ == "__main__":
    ...
    for i in range(marble_cnt+1):
        if i % 10000 == 0:
            print(i)
        if i != 0 and i % 23 == 0:
            cur_marble = (cur_marble - 7) % len(marble_circle)
            player_scores[cur_player] += i + marble_circle.pop(cur_marble)
        else:
            if i != 0:
                cur_marble = (cur_marble + 2) % len(marble_circle)
            else:
                cur_marble = 0
            marble_circle.insert(cur_marble, i)
