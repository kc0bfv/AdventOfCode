...
fn do_output<F>(prgm_index: usize, opcode: i64, memrep: &mut MemRep, out_vals: &mut Vec<i64>, mut output_func: F)
    where F: FnMut(i64) -> Result<(), &'static str>,
{
    let modes = get_param_modes(opcode, 1);
    let out_val: i64 = handle_read_param(memrep, 1, &modes[0]);
    println!("Prgm {} Output: {}", prgm_index, out_val);
    out_vals.push(out_val);
    match output_func(out_val) {
...
pub fn run_program<I, F>(prgm_index: usize, initial_memory: &Vec<i64>,
        mut input_iter: &mut I, mut output_func: F) -> (Vec<i64>, Vec<i64>)
    where I: Iterator<Item = i64>,
            F: FnMut(i64) -> Result<(), &'static str>,
{
    ...
    while memrep.cur_pos < initial_memory.len() {
        ...
        match opcode % 100 {
            ...
            3 => do_input(prgm_index, opcode, &mut memrep, &mut input_iter),
            4 => do_output(prgm_index, opcode, &mut memrep, &mut out_vals, &mut output_func),
