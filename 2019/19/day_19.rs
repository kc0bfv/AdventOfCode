...
impl Drone {
    ...
    fn calc_next_part_2_edge(&mut self) -> Position {
        ...
        while self.p2_edge_mode {
            let checking_y = self.last_le.y + 1;
            let mut checking_le_x = self.last_le.x;
            let mut checking_re_x = self.last_re.x;
        
            loop {
                let le_check = Position::new(checking_le_x, checking_y);
                if !self.is_explored(&le_check) {
                    return le_check;
                } else if self.is_trac(&le_check) {
                    break;
                }
                checking_le_x += 1;
            }
            loop {
                let re_check = Position::new(checking_re_x, checking_y);
                if !self.is_explored(&re_check) {
                    return re_check;
                } else if !self.is_trac(&re_check) {
                    checking_re_x -= 1;
                    break;
                }
                checking_re_x += 1;
            }
            self.last_le = Position::new(checking_le_x, checking_y);
            self.last_re = Position::new(checking_re_x, checking_y);

            if (self.last_re.x - self.last_le.x) >= (box_size - 1) {
                self.p2_edge_mode = false;
            }
        }

        // Length finding mode
        // When edge finding mode worked, now track the left edge to make 
        // sure it stays >= 100 points apart from the old right edge's x for
        // 100 points
        ...
        // If bottom left corner is track, we're done.
            ...
            println!("Part 2: {}", 
                    (self.last_re.x - (box_size - 1)) * 10000 + self.last_le.y);
